...
**实时协作驾驶舱（2.7.0）**：TUI 驾驶舱现在是运行中协作的实时控制面：
横排管线渲染每个 agent 槽位，连接符即真值（`──→` 观察到的 HANDOFF、
`┄┄→` 计划相邻），方向键选中、Enter 展开单个 agent 的有界详情窗、
`L` 一键切换中英文界面。运行期间可直接转向：键入任意文本即进入持久
转向输入面（无需前缀键），`tab` 切换修订目标（下轮调用 / 提交提示词），
Enter 经既有控制面提交修订（`p` 暂停 / `r` 恢复 / `a` 中止 / `t` 追踪 /
`q` 退出），提交后输入面保持打开、支持连续修订——每笔提交都回以诚实
回执（accepted ≠ applied：applied 事实只落在追踪视图，主屏绝不宣称）。
`t` 打开只读观察追踪：整个运行的控制账本、修订队列与逐调用事件。
更丰富的界面没有牺牲渲染性能——投影带变更检测与分区作用域，键入
只触碰 dock 行（每次键击零全量投影）。

**对话式协作驾驶舱（2.8.0）**：驾驶舱升级为多轮对话——一轮协作终态后
直接键入下一个任务，同一会话启动下一轮（log 分节线分隔各轮；每轮全新
身份/事实/控制状态；不输入即退出与从前完全一致，退出码仍映射末轮
结局）。新增会话命令：`/again` 经同一池门+指纹门载入上轮任务、
`/compose` 重开 runtime 选择屏、`/runs` 列出会话全部轮次、`/new`
清空呈现史（引擎事实不动）。多组组合横排并列、组间连接符同为真值
（`──→` 观察 / `┄┄→` 计划），选择屏 `g` 键分配成员分组，`H` 显式
横滚守门超大布局。深化焦点：宽屏 `d` 开右侧有界详情栏，展开格上
`x`/Enter 跳入追踪页并预选该成员。`/runs` 追加每轮详情块——组合、
